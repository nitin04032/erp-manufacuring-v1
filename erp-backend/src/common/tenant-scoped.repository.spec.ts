...
describe('TenantScopedRepository', () => {
  const companyId = 7;
  let repo: jest.Mocked<Pick<Repository<FakeEntity>, 'find' | 'findOne' | 'count' | 'create' | 'save' | 'softDelete'>>;
  ...
  beforeEach(() => {
    ...
    scoped = new TenantScopedRepository(repo as unknown as Repository<FakeEntity>, companyId);
  });
  ...
  it('find() merges company_id into an existing where object without overriding other fields', async () => {
    ...
    expect(repo.find).toHaveBeenCalledWith(
      expect.objectContaining({ where: { company_id: companyId, name: 'widget' } }),
    );
  });
  ...
  it('create() stamps company_id onto the new entity', () => {
    ...
    expect(repo.create).toHaveBeenCalledWith({ name: 'widget', company_id: companyId });
  });

  it('save() stamps company_id when absent', async () => {
    await scoped.save({ name: 'widget' } as any);
    expect(repo.save).toHaveBeenCalledWith({ name: 'widget', company_id: companyId });
  });
  ...
  it('save() throws rather than silently cross-writing a different company_id', async () => {
    await expect(scoped.save({ name: 'widget', company_id: 999 } as any)).rejects.toThrow(
      /refusing to save/,
    );
  ...
  it('softDelete() scopes by company_id', async () => {
    ...
    expect(repo.softDelete).toHaveBeenCalledWith({ company_id: companyId, id: 5 });
  });
